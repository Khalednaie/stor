...
exports.list = (req,res)=>{
    ...
    Product.find()
      ...
      .limit(limit)
      .exec((err,products)=>{
        //   console.log(products,'is a product')
          if(err){
              console.log('err in all list produ',err)
              return res.status(400).json({
                  error:'product not found',
                  typeerr: errorHandler(err)
              })
          }
          res.send(products);
      })
...
/**
 * it will find the products based the req product category
...
exports.listCategories=(req,res)=>{
   Product.distinct('category',{},(err,categores)=>{
       if(err){
           console.log("error in listcategoris",err)
           return res.status(400).json({
               error:'categores not found'
           })
        }
        res.json(categores)
   })
